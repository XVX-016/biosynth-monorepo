...
export default function PropertiesPanel() {
	...
	const content = useMemo(() => {
		if (!molecule) {
			return <div className="text-text-secondary">No molecule loaded.</div>;
		}
		if (selectedAtomId) {
			...
			return (
				<div>
					<div className="text-sm text-text-secondary mb-1">Selected Atom</div>
					<div className="text-xl font-semibold">{atom.element}</div>
					<div className="text-sm text-text-secondary mt-2">
						Position: ({atom.position[0].toFixed(2)}, {atom.position[1].toFixed(2)},{' '}
						{atom.position[2].toFixed(2)})
					</div>
					<div className="text-sm text-text-tertiary mt-1">ID: {atom.id}</div>
				</div>
			);
		}
		if (selectedBondId) {
			...
			return (
				<div>
					<div className="text-sm text-text-secondary mb-1">Selected Bond</div>
					...
					<div className="text-sm text-text-tertiary mt-1">ID: {bond.id}</div>
				</div>
			);
		}
		return <div className="text-text-secondary">Select an atom or bond to view details.</div>;
